...
def create_target(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Garante que defas seja numérico
    df["defas"] = pd.to_numeric(df["defas"], errors="coerce")

    # MELHORIA: Remover linhas onde 'defas' é NaN antes de criar o target.
    initial_rows = len(df)

    # CORREÇÃO APLICADA: Adicionado .copy() para evitar SettingWithCopyWarning
    df = df.dropna(subset=["defas"]).copy()

    dropped_rows = initial_rows - len(df)

    if dropped_rows > 0:
        logger.warning(f"Removidas {dropped_rows} linhas com 'defas' nulo/inválido.")

    # 1. Criação do alvo binário (Defasagem negativa = Atraso = Risco 1)
